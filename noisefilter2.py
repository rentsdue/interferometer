...
for fname, theta_a_deg, theta_b_deg in file_specs:
    ...
    # Plot all raw peaks (before filtering)
    ...
    # Plot filtered peaks (after separation)
    if len(idx) > 0:
    ...
    if m_meas > 0:
        plt.title(f"Peak Detection (m={m_meas}, λ≈{2*d_slab_m/m_meas:.2e} m)",
                fontsize=18)
    else:
        plt.title("Peak Detection (no valid peaks)", fontsize=18)
    ...
    if np.isfinite(n_est):
        print(f"{fname}: m = {m_meas}, n = {n_est:.5f}")
    else:
        print(f"{fname}: m = {m_meas}, n = NaN")
